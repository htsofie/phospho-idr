...
def process_human_interfaces(interfaces_df, human_interface_file):
    ...
    IMPORTANT: When a match is found for a human_ortholog_id, it creates rows
    for ALL protein_ids that share that human_ortholog_id, not just the current row.
    ...
    # Build mapping of human_ortholog_id -> list of all rows (protein_ids) that have it
    # This ensures we propagate matches to all protein_ids sharing the same ortholog
    ortholog_to_rows = {}
    rows_without_ortholog = []
    for idx, row in interfaces_df.iterrows():
        human_ortholog_id = row['human_ortholog_id']
        if pd.notna(human_ortholog_id):
            if human_ortholog_id not in ortholog_to_rows:
                ortholog_to_rows[human_ortholog_id] = []
            ortholog_to_rows[human_ortholog_id].append((idx, row))
        else:
            rows_without_ortholog.append(row)
    
    logger.info(f"Found {len(ortholog_to_rows)} unique human ortholog IDs")
    
    # List to store new rows (for multiple matches)
    new_rows = []
    # Track seen interactions to avoid duplicates
    seen_interactions = set()
    # Track which orthologs have matches
    orthologs_with_matches = set()
    
    logger.info("Matching human ortholog IDs to interfaces...")
    # Process each unique human_ortholog_id once
    for human_ortholog_id, ortholog_rows in ortholog_to_rows.items():
        ...
        # Process P1 matches - create rows for ALL protein_ids with this ortholog
        for _, match in p1_matches.iterrows():
            # Create unique key for this interaction to avoid duplicates
            ...
            matches_found = True
            # Create rows for ALL protein_ids that share this human_ortholog_id
            for ortholog_idx, ortholog_row in ortholog_rows:
                new_row = ortholog_row.copy()
        ...
        # Process P2 matches - create rows for ALL protein_ids with this ortholog
        for _, match in p2_matches.iterrows():
            # Create unique key for this interaction to avoid duplicates
        ...
        # Track which orthologs had matches
        if matches_found:
            orthologs_with_matches.add(human_ortholog_id)
        
        # For orthologs without matches, keep original rows as-is
        if not matches_found:
            for ortholog_idx, ortholog_row in ortholog_rows:
                new_rows.append(ortholog_row)
        else:
            # For orthologs with matches, also keep original rows if they have mouse interactions
            # This preserves mouse interaction data when both mouse and human matches exist
            for ortholog_idx, ortholog_row in ortholog_rows:
                if 'mouse_interactor_id' in ortholog_row and pd.notna(ortholog_row.get('mouse_interactor_id')):
                    # Keep original row with mouse interactions (human columns remain None)
                    new_rows.append(ortholog_row)
    
    # Add rows without ortholog IDs
    new_rows.extend(rows_without_ortholog)
